...
def costs_chart(df, C_buy, C_sell, actual_cost, T, scenario_name, results_folder):
    ...
    plt.savefig(
        f"{results_folder}/{scenario_name.replace(' ', '_')}_costs.png", dpi=200
    )
...
def sources_chart(df, scenario_name, results_folder):
    ...
    plt.plot(
        hours, df["Demand"], "k-", linewidth=3, marker="o", markersize=4, label="Demand"
    )

    plt.title(
        f"{scenario_name} - Energy Sources Supplying the Load (24h)",
        fontsize=14,
        pad=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{results_folder}/{scenario_name.replace(' ', '_')}_sources.png", dpi=300
    )
    plt.close()
...
def decision_variables_chart(df, scenario_name, results_folder):
    ...
    plt.tight_layout()
    plt.savefig(
        f"{results_folder}/{scenario_name.replace(' ', '_')}_binary_decisions.png",
        dpi=300,
    )
...
def battery_charging_chart(df, scenario_name, results_folder):
    ...
    plt.tight_layout()
    plt.savefig(
        f"{results_folder}/{scenario_name.replace(' ', '_')}_battery_behavior.png",
        dpi=300,
    )
    plt.close()
...
def comparison_chart(df_lin_A, df_lin_B, df_nonlin_A, df_nonlin_B, results_folder):
    ...
    lin_A = [
        df_lin_A["Solar"].sum(),
        df_lin_A["Buy"].sum(),
        df_lin_A["Discharge"].sum(),
        df_lin_A["Sell"].sum(),
    ]
    lin_B = [
        df_lin_B["Solar"].sum(),
        df_lin_B["Buy"].sum(),
        df_lin_B["Discharge"].sum(),
        df_lin_B["Sell"].sum(),
    ]
    nonlin_A = [
        df_nonlin_A["Solar"].sum(),
        df_nonlin_A["Buy"].sum(),
        df_nonlin_A["Discharge"].sum(),
        df_nonlin_A["Sell"].sum(),
    ]
    nonlin_B = [
        df_nonlin_B["Solar"].sum(),
        df_nonlin_B["Buy"].sum(),
        df_nonlin_B["Discharge"].sum(),
        df_nonlin_B["Sell"].sum(),
    ]
    ...
    ax.bar(
        ...
        lin_A,
        width,
        label="(Linear) - Scenario 1 - 22 AMD",
        color="#3498db",
        alpha=0.9,
    )
    ax.bar(
        ...
        lin_B,
        width,
        label="(Linear) - Scenario 2 - 35/48 AMD",
        color="#04548a",
        alpha=0.9,
    )
    ax.bar(
        ...
        nonlin_A,
        width,
        label="(Non-linear) - Scenario 1 - 22 AMD",
        color="#e74c3c",
        alpha=0.9,
    )
    ax.bar(
        ...
        nonlin_B,
        width,
        label="(Non-linear) - Scenario 2 - 35/48 AMD",
        color="#ac2516",
        alpha=0.9,
    )
...
def sensitivity_chart(solve_scenario, scenario_name, results_folder):
    ...
    plt.plot(
        prices,
        [pr / 1000 for pr in actual_costs],
        "s-",
        color="purple",
        label="Actual Cost (thousands AMD)",
    )
    plt.plot(
        prices,
        [baseline_cost / 1000 for _ in prices],
        linewidth=2,
        linestyle="--",
        color="black",
        label="Baseline Cost (thousands AMD)",
    )
    plt.axvline(
        x=22,
        color="#c0392b",
        linewidth=2,
        linestyle="--",
        label="Current sell price (22 AMD/kWh)",
    )
